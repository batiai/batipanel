#!/usr/bin/env bash
# install.sh - batipanel 설치 스크립트

set -euo pipefail

echo "batipanel - AI 개발 워크스페이스 세팅 시작..."
...
for script in common.sh layout_6panel.sh layout_7panel.sh layout_7panel_log.sh start.sh example.sh; do
...
ALIAS_LINE="alias b='bash ~/tmux/start.sh'"
if ! grep -q "alias b=" "$SHELL_RC" 2>/dev/null; then
  ...
  echo "  alias b 등록됨 ($SHELL_RC)"
else
  echo "  alias b 이미 등록됨"
fi

echo ""
echo "batipanel 설치 완료!"
echo ""
echo "사용법:"
echo "  b myproject                  # 프로젝트 시작 or 복귀"
echo "  b myproject --layout 6panel  # 레이아웃 지정"
echo "  b new 이름 경로              # 새 프로젝트 등록"
echo "  b stop myproject             # 세션 종료"
echo "  b ls                         # 목록 확인"
echo "  b layouts                    # 레이아웃 목록"
